...
public class Validator {
    ...
    public void validate (String [] data){
        ...
        if (flag == false){
            throw new UncorrectData(sb.toString());
        } 
    }
    ...
    private boolean isValidName (String name){
        for (int i = 0; i < name.length(); i++) {
            if (! Character.UnicodeBlock.of(name.charAt(i)).equals(Character.UnicodeBlock.CYRILLIC)) {
                throw new UncorrectData(String.format("некорректно задано имя"));
            }
        }
        return true;
    }
    ...
    private boolean isValidDate (String birthday)  {
        ...
        try {
            ...
            throw new UncorrectData("некорректный формат даты");
        }
        ...
        if ((Arrays.asList(month_30).contains(date.getMonthValue()) && day > 30) ||
            ...
            throw new UncorrectData("введена некорректная дата рождения");
